...
//    }
    ...
    @GetMapping("/getSharedFileInfo")
    @Operation(summary = "通过 access_token 获取分享文件信息")
    public R getSharedFileInfo(@RequestParam("access_token") String accessToken) {

        if (accessToken == null || accessToken.trim().isEmpty()) {
            return R.fail().message("access_token 不能为空");
        }
        ...
        // 构造返回对象（与 readQRCode 保持一致）
        QRDecodeFileInfo qrDecodeFileInfo = new QRDecodeFileInfo();
        qrDecodeFileInfo.setFileName(ypFile.getFileName());
        qrDecodeFileInfo.setFileId(ypFile.getId());
        System.out.println("<UNK>id<UNK>" + ypFile.getId());
        qrDecodeFileInfo.setFileSize(ypFile.getFileSize());
        qrDecodeFileInfo.setFileType(ypFile.getFileType());
        qrDecodeFileInfo.setOwnerName(ownerName);
        qrDecodeFileInfo.setDownloadTimes(ypFile.getDownloadTimes());
        qrDecodeFileInfo.setFileImg(ypFile.getFileImg());
        // 注意：downloadUrl 应该是完整的下载地址，前端会用它下载
        qrDecodeFileInfo.setDownloadUrl("http://localhost:8989/publicDownload?access_token=" + accessToken);
    ...
    @Operation(summary = "非本平台下载分享文件（暂时不用）")
    ...
    //生成二维码，使用其他平台识别二维码时，可以直接下载
    @Operation(summary = "获取二维码1")
    @GetMapping(value = "/createQRCode1")
    public byte[] createQRCode1(@RequestParam("fileId")Long fileId) throws IOException, WriterException {//测试成功
        ...
        String note = "";
        String tui = "";
        CreateQRCodeWithBG.generateQRCodeImage(url, 300, 300, qrcodewithbgPath);
        if(!qrCodeFile.exists()){//如果要生成的分享文件不存在
            System.out.println("二维码生成失败");
            //return R.fail().message("二维码生成失败");
        }
