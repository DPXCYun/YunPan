...
public class RecyclebinController {
     ...
     * @return
     */
    @Operation(summary = "删除回收站文件夹或文件")
    @DeleteMapping("/deleteBinFolder")
    public R deleteFolder(Long folderId){
    ...
    @GetMapping("/getBinFilesAndFolders")
     ...
     * @return
     */
    @Operation(summary = "bin根路径陈列")
    @GetMapping("/getFilesByBinPath")
    public R getFilesByBinPath(@RequestParam String path){
        Long userId = StpUtil.getLoginIdAsLong();
    ...
    public R recoverFile(Long fileId){
        Long userId = StpUtil.getLoginIdAsLong();
        YPRecyclebin ypBinFile = recyclebinService.getBinFileById(fileId);
        if(ypBinFile == null || !userId.equals(ypBinFile.getUserId())){
            log.error(String.format("userID=%d, binFile not fount:%d", userId, fileId));
     ...
     * @return
     */
    @Operation(summary = "恢复文件夹或文件")
    @PutMapping("/recoverFolder")
    public R recoverFolder(Long folderId){
        Long userId = StpUtil.getLoginIdAsLong();
