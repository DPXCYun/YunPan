...
public class FileQueryController {
    ...
    public R deleteFile(Long fileId){
        Long userId = StpUtil.getLoginIdAsLong(); // 从 token 安全获取
    ...
    public R renameFile(String newFileName, Long fileId){
     ...
     * @return
     */
    @Operation(summary = "文件夹或文件重命名")
    @PutMapping("/rename")
    public R renameFolder(Long folderId, String newFolderName){
...
//        }
     ...
     * @return
     */
    @Operation(summary = "删除文件夹或文件")
    @DeleteMapping("/delete")
    public R deleteFolder(Long fileId){
...
//                        System.out.println("folderBinPath:"+binPath);
    ...
    public R createFolder(String path, String folderName){
        Long userId = StpUtil.getLoginIdAsLong(); // 从 token 安全获取
        //判断同级文件夹是否重名
     ...
     * @return
     */
    @Operation(summary = "根据路径查询路径下所有文件和文件夹")
    @GetMapping("/getFilesAndFolders")
    public R getFilesByFolderPath(@RequestParam String path){
        Long userId = StpUtil.getLoginIdAsLong(); // 从 token 安全获取
    ...
    public R getFolderSize(@RequestParam String path){
        Long userId = StpUtil.getLoginIdAsLong(); // 从 token 安全获取
        List<YPFile> folders = fileQueryService.getFolderListBySameLevelVirtualPath(path, userId);
...
//            System.out.println(size);
    ...
    @PutMapping("/moveFile")
    public  R moveFile(Long fileId, String newPath){
        Long userId = StpUtil.getLoginIdAsLong(); // 从 token 安全获取
        YPFile ypFile = fileQueryService.getFileById(fileId);
        if(ypFile == null || !userId.equals(ypFile.getUserId())){
            log.error(String.format("userID=%d, file not fount:%d", userId, fileId));
     ...
     * @return
     */
    @Operation(summary = "移动文件夹或文件")
    @PutMapping("/move")
    public  R moveFolder(Long folderId, String newPath){
        Long userId = StpUtil.getLoginIdAsLong(); // 从 token 安全获取
